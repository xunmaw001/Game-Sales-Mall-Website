<template>
  <div class="addEdit-block">
    ...
    >
      <el-row>
                        <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="关联表id" prop="refid">
          <el-input v-model="ruleForm.refid" 
              placeholder="关联表id" clearable ></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="关联表id" prop="refid">
              <el-input v-model="ruleForm.refid" 
                placeholder="关联表id" readonly></el-input>
                                    ...
                                    <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="评论内容" prop="content">
          <el-input v-model="ruleForm.content" 
              placeholder="评论内容" clearable ></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="评论内容" prop="content">
              <el-input v-model="ruleForm.content" 
...
export default {
  ...
  methods: {
        ...
        for (var o in obj){
            ...
            continue;
                                ...
                                // 多级联动参数
                                          info(id) {
      this.$http({
        url: `discussremenyouxipaixingbang/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
    ...
    onSubmit() {
        ...
        if (valid) {
          this.$http({
            url: `discussremenyouxipaixingbang/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.discussremenyouxipaixingbangCrossAddOrUpdateFlag = false;
                  this.parent.search();
